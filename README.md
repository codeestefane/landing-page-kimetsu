...
1. Para ler o arquivo ``docker-compose.yml``, subir todos os containers especificados (wordpress, database e phpmyadmin) e liberar o terminal para outras aplicações.
```
docker-compose up -d
```

2. Para interromper a execução dos containers.
```
docker-compose down
```  
